...
if [ "${component}" == "all" ]; then
  ...
  for component in frontend mongodb catalogue redis user cart mysql shipping rabbitmq payment; do
      ...
      aws ec2 run-instances  --launch-template LaunchTemplateId=lt-06383d1526d457148 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${component}}]" &>/tmp/{component}.log
      sleep 5
      fi
...
if [ "$STATE" != "running" ]; then
  aws ec2 run-instances  --launch-template LaunchTemplateId=lt-06383d1526d457148 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${component}}]" | jq .
  sleep 15
fi
